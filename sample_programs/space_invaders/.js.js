(function() {
  var Bitmap, BoundaryManager, Circle, CollisionManager, ColorTween, Curve, GameLoop, Gradient, KeyManager, Line, Matrix, Pattern, Polygon, Rectangle, RoundedRectangle, Scene, Shape, Sprite, Square, Star, Text, Triangle, Tween, animations, collisions, enemies, enemy, enemy1, enemy10, enemy11, enemy12, enemy13, enemy14, enemy2, enemy3, enemy4, enemy5, enemy6, enemy7, enemy8, enemy9, f, gameLoop, gun, include, keys, namespace, row1Enemies, row2Enemies, row3Enemies, scene, shot, tween1, tween2, tween3, tween4, _fn, _fn2, _fn3, _i, _j, _k, _l, _len, _len2, _len3, _len4, _len5, _len6, _m, _n,
  ...
  BoundaryManager = (function() {

    function BoundaryManager(width, height) {
      this.width = width;
      this.height = height;
      this.sprites = [];
      this.collided = [];
    ...
    BoundaryManager.prototype.onUpdate = function(event) {
      var boundaryEvent, sprite, _i, _len, _ref;
      this.thisTime = new Date();
      _ref = this.collided;
      for (_i = 0, _len = _ref.length; _i < _len; _i++) {
        sprite = _ref[_i];
        ...
        sprite.receiveBoundaryEvent(this.thisTime.getTime() - this.lastTime.getTime());
      }
      this.collided = [];
      return this.lastTime = this.thisTime;
    };

    BoundaryManager.prototype.observe = function(sprite) {
      return this.sprites.push(sprite);
    };

    BoundaryManager.prototype.detectCollisions = function(o1) {
      if (o1.x >= this.width) return true;
      if (o1.x <= 0) return true;
      if (o1.y >= this.height) return true;
      if (o1.y <= 0) return true;
      return false;
    };

    BoundaryManager.prototype.runBoundaryDetection = function() {
      var sprite, _i, _len, _ref, _results;
      _ref = this.sprites;
      _results = [];
      for (_i = 0, _len = _ref.length; _i < _len; _i++) {
        sprite = _ref[_i];
        if (this.detectCollisions(sprite) === true) {
          _results.push(this.collided.push(sprite));
        } else {
          _results.push(void 0);
        }
      }
  ...
  namespace('CollisionResponse', function(exports) {
    ...
    exports.randomBounce = function(o) {
  ...
  for (_i = 0, _len = row1Enemies.length; _i < _len; _i++) {
    enemy = row1Enemies[_i];
    scene.makeChild(enemies, enemy);
  }

  for (_j = 0, _len2 = row2Enemies.length; _j < _len2; _j++) {
    enemy = row2Enemies[_j];
    scene.makeChild(enemies, enemy);
  }

  for (_k = 0, _len3 = row3Enemies.length; _k < _len3; _k++) {
    enemy = row3Enemies[_k];
    scene.makeChild(enemies, enemy);
  }
  ...
  animations = {
    changeVelocity: function() {
      console.log("moo!!!!");
      if (this.adjustedY < 350) {
        this.xVelocity = this.xVelocity * -1;
        return this.move(10 * this.xVelocity, .5 + this.y / 100);
      } else {
        return alert("You Lost!");
      }
    },
    bloop: function() {
      shot.moveTo(this.x, this.y - this.height / 2 - 20);
      return shot.yVelocity = -.6;
    },
    splode: function() {
      return scene.removeChild(enemies, this);
    }
  };
  ...
  _fn = function(enemy) {
    ...
    enemy.addEvent({
      ...
      objectType: "boundary",
      toDo: f = function() {
        return enemy.changeVelocity();
      }
    });
    return enemy.addAnimation(tween1);
  };
  for (_l = 0, _len4 = row1Enemies.length; _l < _len4; _l++) {
    enemy = row1Enemies[_l];
    _fn(enemy);
  }
  ...
  _fn2 = function(enemy) {
    ...
    return enemy.addAnimation(tween3);
  };
  for (_m = 0, _len5 = row2Enemies.length; _m < _len5; _m++) {
    enemy = row2Enemies[_m];
    _fn2(enemy);
  }
  ...
  _fn3 = function(enemy) {
    ...
    return enemy.addAnimation(tween4);
  };
  for (_n = 0, _len6 = row3Enemies.length; _n < _len6; _n++) {
    enemy = row3Enemies[_n];
    _fn3(enemy);
  }
